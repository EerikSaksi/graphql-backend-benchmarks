...
init () {
	docker run --network=host --name hasura-chinook -e PGPoolSettings="{max_connections:1}" -e HASURA_GRAPHQL_DATABASE_URL="${DATABASE_URL}?ssl=true" -e HASURA_GRAPHQL_ENABLE_CONSOLE=true -d hasura/graphql-engine:latest graphql-engine serve 
	sleep 10
